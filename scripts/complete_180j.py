"""Complete the 180j program that was interrupted."""
import sys
...
# Get the 180j program
progs = get("programs", {"type": "eq.180j", "select": "id"})
prog_id = progs[0]["id"] if progs else None
print(f"180j program ID: {prog_id}")
...
print(f"Existing days: 1-{max_day}, need to complete up to 180")
...
courses_per_day = max(1, math.ceil(total_courses / 180))
print(f"Total courses: {total_courses}, {courses_per_day} per day")

# Create missing days (from max_day+1 to 180)
created = 0
for day_num in range(max_day + 1, 181):
...
print(f"\nCreated {created} more days. 180j program is now complete!")
