  ...
  return (
    <VisualWrapper>
      <svg viewBox="0 0 280 200" style={{ width: '100%', maxWidth: '280px' }}>
        {/* Central node */}
        <ellipse cx="140" cy="100" rx="36" ry="22" fill="none" stroke="#3B82F6" strokeWidth="1.5" />
        <text x="140" y="105" textAnchor="middle" fontSize="11" fill="#3B82F6" fontFamily="Outfit, system-ui" fontWeight="600">Cours</text>
        {/* Branches */}
        {[
          ...
          <g key={i}>
            <line x1="140" y1="100" x2={x2} y2={y2} stroke="#E5E5E5" strokeWidth="1.2" />
            <ellipse cx={cx} cy={cy} rx="24" ry="14" fill="white" stroke="#D4D4D4" strokeWidth="1" />
            <text x={cx} y={cy + 4} textAnchor="middle" fontSize="9" fill="#737373" fontFamily="Outfit, system-ui">{label}</text>
          </g>
        ))}
      </svg>
    </VisualWrapper>
    ...
    <VisualWrapper>
      <div style={{ width: '100%', maxWidth: '300px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: '4px', height: '60px', marginBottom: '20px', justifyContent: 'center' }}>
          {[35, 60, 45, 80, 55, 90, 40, 75, 50, 85, 38, 70, 52, 88, 44].map((h, i) => (
            <div key={i} className="animate-waveform" style={{
              flex: 1, background: '#3B82F6', borderRadius: '2px',
        ...
        <div style={{ background: 'white', border: '1px solid #E5E5E5', borderRadius: '12px', padding: '16px' }}>
          <div style={{ fontFamily: 'Outfit, system-ui', fontSize: '13px', fontWeight: 600, color: '#0A0A0A', marginBottom: '8px' }}>Anatomie — Cours 04</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ flex: 1, height: '3px', background: '#E5E5E5', borderRadius: '2px' }}>
              <div style={{ width: '38%', height: '100%', background: '#3B82F6', borderRadius: '2px' }} />
            </div>
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '11px', color: '#A3A3A3' }}>4:12</span>
          </div>
        </div>
      </div>
    </VisualWrapper>
...
function QCMVisual() {
  ...
  return (
    <VisualWrapper>
      <div style={{ width: '100%', maxWidth: '300px' }}>
        <div style={{ fontFamily: 'Outfit, system-ui', fontSize: '13px', fontWeight: 600, color: '#0A0A0A', marginBottom: '16px', lineHeight: 1.5 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {OPTIONS.map((opt, i) => (
            <div key={i} style={{
              ...
              border: i === 1 ? '1.5px solid #10B981' : '1px solid #E5E5E5',
              ...
              <span style={{ fontFamily: 'Outfit, system-ui', fontSize: '12px', color: i === 1 ? '#10B981' : '#737373' }}>{opt}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <VisualWrapper>
      <div style={{ position: 'relative', width: '100%', maxWidth: '280px' }}>
        {/* Stack effect */}
        <div style={{ position: 'absolute', top: 6, left: 6, right: -6, bottom: -6, background: '#E5E5E5', borderRadius: '14px' }} />
        ...
        <div style={{
          ...
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '10px', color: '#A3A3A3', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '16px' }}>Définition</div>
          <div style={{ fontFamily: 'Outfit, system-ui', fontSize: '15px', fontWeight: 600, color: '#0A0A0A', lineHeight: 1.5, marginBottom: '20px' }}>
            Qu&apos;est-ce que la plasticité synaptique ?
          </div>
          <div style={{ height: '1px', background: '#F1F1F1', marginBottom: '16px' }} />
          <div style={{ fontFamily: 'Outfit, system-ui', fontSize: '12px', color: '#737373', lineHeight: 1.6 }}>
    ...
    <VisualWrapper>
      <div style={{ width: '100%', maxWidth: '280px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div style={{ fontFamily: 'Outfit, system-ui', fontSize: '13px', fontWeight: 600, color: '#0A0A0A' }}>Examen Blanc #3</div>
        ...
        <div style={{ marginBottom: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
            <span style={{ fontFamily: 'Outfit, system-ui', fontSize: '12px', color: '#737373' }}>Progression</span>
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '12px', color: '#3B82F6' }}>34 / 60</span>
          </div>
          <div style={{ height: '6px', background: '#F1F1F1', borderRadius: '3px', overflow: 'hidden' }}>
            <div style={{ width: '57%', height: '100%', background: '#3B82F6', borderRadius: '3px' }} />
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
          {[['28', 'Correctes', '#10B981'], ['6', 'Erreurs', '#F43F5E'], ['0', 'Restantes', '#A3A3A3']].map(([n, l, c]) => (
            <div key={l} style={{ background: 'white', border: '1px solid #E5E5E5', borderRadius: '10px', padding: '12px', textAlign: 'center' }}>
              <div style={{ fontFamily: 'Outfit, system-ui', fontSize: '20px', fontWeight: 700, color: c }}>{n}</div>
              <div style={{ fontFamily: 'Outfit, system-ui', fontSize: '10px', color: '#A3A3A3', marginTop: '2px' }}>{l}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    <VisualWrapper>
      <div style={{ width: '100%', maxWidth: '280px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
          <div style={{ fontFamily: 'Outfit, system-ui', fontSize: '13px', fontWeight: 600, color: '#0A0A0A' }}>Cette semaine</div>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {DAYS.map((d, i) => (
            <div key={i} style={{ flex: 1, textAlign: 'center', fontFamily: "'JetBrains Mono', monospace", fontSize: '10px', color: i === 5 ? '#3B82F6' : '#A3A3A3' }}>{d}</div>
          ))}
        </div>
      </div>
    </VisualWrapper>
  ...
  return (
    <section id="outils" ref={ref} style={{ background: '#FFFFFF', padding: '120px 0' }}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', borderBottom: '1px solid #E5E5E5', marginBottom: '48px', overflowX: 'auto', gap: '0' }}>
          {TABS.map((t, i) => (
            <button key={t.id} onClick={() => setActiveTab(i)} style={{
              fontFamily: "'JetBrains Mono', monospace",
              fontSize: '13px', fontWeight: 600,
              color: i === activeTab ? '#0A0A0A' : '#A3A3A3',
              ...
              onMouseEnter={(e) => { if (i !== activeTab) (e.currentTarget as HTMLElement).style.color = '#525252' }}
              onMouseLeave={(e) => { if (i !== activeTab) (e.currentTarget as HTMLElement).style.color = '#A3A3A3' }}
            >
              {t.num} {t.label}
            </button>
          ))}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div>
              ...
              <h3 style={{ fontFamily: 'Outfit, system-ui, sans-serif', fontSize: '28px', fontWeight: 700, color: '#0A0A0A', marginBottom: '16px' }}>{tab.title}</h3>
              <p style={{ fontFamily: 'Outfit, system-ui, sans-serif', fontSize: '15px', color: '#737373', lineHeight: 1.75, marginBottom: '28px' }}>{tab.description}</p>
              <ul style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {tab.points.map((pt) => (
                  <li key={pt} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontFamily: 'Outfit, system-ui, sans-serif', fontSize: '14px', fontWeight: 500, color: '#525252' }}>
                    <Check size={16} weight="bold" style={{ color: '#3B82F6', flexShrink: 0 }} />
                    {pt}
                  </li>
                ))}
              </ul>
